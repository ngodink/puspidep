...
$(".form-confirm").on("submit", (() => {
	var e = confirm("Apakah Anda yakin?");
	if(!e) $.unblockUI();
	return e;
}));

$('.navbar-nav.sidebar a.collapse-item').each((i, e) => {
	if(window.location.href.includes(e.href)) {
		let el = $(e).addClass('font-weight-bold').parents('.nav-item').addClass('active');
		if ($(window).width() >= 768)
			el.find('.collapse').addClass('show')
	}
})